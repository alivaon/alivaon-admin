...
SEO_BASIC_AUTH='utilisateur:motdepasse' pnpm seo:snapshot \
  ...
  --seeds-from prod-2026-09-23 \
  --label _work/staging
pnpm seo:compare --baseline prod-2026-09-23 --target _work/staging
```
...
| Relevé | Contenu |
|---|---|
| `tests/seo-baseline/prod-2026-09-23/` | Production avant toute modification — **référence de la migration** |
...
6. Performance mobile faible sur tous les gabarits (Lighthouse, médiane de 3
   ...
   0,22 (`/en`). Détail : `tests/seo-baseline/prod-2026-09-23/lighthouse/summary.json`.
